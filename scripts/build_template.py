#!/usr/bin/env python3
"""
Rebuild forecast_baseline_template.xlsx (v3.2) for Lele Aviation.
...
universe (AeroLOPA integration), wide retrofit-default tables.
...
assert len(pricing) == 414, f"expected 414 pricing rows, got {len(pricing)}"
...
sheet_purpose = [
    ...
    ("03_Pricing_Reference", "Read-only: all 414 pricing assumptions with escalated costs for 2028/2030/2032."),
...
plain(ws.cell(row=r,column=2), wrap=True)
...
steps = [
    ...
    ("MANDATORY — known gap, ask Claude to fix", "Turboprop subfleets (73 active) currently have NO pricing "
     "assumptions in 03_Pricing_Reference at all (the assumptions table has rows for Narrowbody/Widebody/"
     "Regional only) — Turboprop spend will not calculate until placeholder Turboprop pricing rows are added "
     "to the database."),
