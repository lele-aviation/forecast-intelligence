...
(cols AK-AN), Turboprop pricing coverage (72 rows added v3.3.1), and full
airline categorization -- 05_Airline_Defaults col B (Category) is now
DB-sourced (987/987 populated, v3.3.2) instead of file-carry-forward.
...
for _r in range(3, _ws_ad.max_row + 1):
    ...
    # Category (col B) is no longer read from the sidecar as of v3.3.2 -- it's
    # sourced directly from airlines.airline_category above (full 987/987 DB
    # categorization, see ARCHITECTURE.md Part 7 item 4 RESOLVED).
    if _notes:
...
changelog_v33 = [
    ...
    "pass, weekend of 2026-06-20/21. 05_Airline_Defaults col B is now sourced directly from "
    "airlines.airline_category in the DB (green fill, DB-sourced/editable) instead of carried forward from "
    "the prior workbook file -- more robust going forward. Cadence Notes (col F) still carry forward "
    "file-to-file as before; there's no dedicated DB column for that free-text field, by design.",
]
