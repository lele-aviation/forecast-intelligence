"""
apply_airline_categories_v2.py
...
Update applies as a single bulk statement (psycopg2 execute_values) rather
than 987 individual round trips -- the per-row version reliably timed out
against the pooled Supabase connection.

Idempotent: safe to re-run (plain UPDATE by name, same input -> same result).

RUN:  python apply_airline_categories_v2.py
"""
...
# Note: cur.rowcount after execute_values only reflects the last internal
# page (default page_size=100), not the true total -- don't rely on it here.
conn.commit()
print(f"   {len(rows)} row values sent in the bulk update (re-running is idempotent -- same values, no-op on a second pass).\n")
