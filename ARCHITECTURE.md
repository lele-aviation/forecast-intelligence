...
> **UPDATE 2026-09-11 — read this before trusting any dollar figure or adding a doc.**
>
> - **This file is the single design record.** `03_Project_Hoku\_Context\data-model.md` is now a
>   pointer plus Hōkū query notes, not a second description of the model. Between 2026-09-09 and
>   2026-09-11 it described engine v2.0's deliberate hard-goods scope as a bug; two days were lost
>   chasing it and the proposed remedy would have destroyed newer work. Do not create a second
>   description of the model anywhere.
> - **Engine v2.0 scope is deliberate.** `calculate_forecast_spend.py` zeroes hard goods (galleys,
>   galley inserts, lavatories, laminates, panels, bins, shades, lighting, monuments, stowage,
>   engineering & certification). Stated in its own docstring. Extending to all 92 SKUs is approved
>   but **not built** — workstream W4.
> - **Snapshots exist** — point-in-time freezes of the model for bespoke engagements and annual
>   locks, in the `snapshots` schema. See **`SNAPSHOTS.md`**. Client builds should read
>   `snapshots.*` with a pinned `snapshot_id`, never `public.*`.
> - **⛔ Freeze in force:** do not run `resync_materials.py --insperial` or the engine directly — it
>   will zero the Insperial laminates ($2.89B + $3.45B) now sitting in `forecast_spend`. See the top
>   of `Outputs\Cabin_Material_Master_2026-08-24\RESYNC_CHECKLIST.md`. Workstream W3.
> - **`forecast_versions` 1 and 2 hold zero rows.** The annual-lock mechanism has never carried data;
>   the 2027–2036 lock due 2026-11-30 is its first real exercise.
> - Full context: `03_Project_Hoku\_Context\Lele_Operating_Architecture_2026-09-10.md`.
