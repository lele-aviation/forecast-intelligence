#!/usr/bin/env python3
"""
Rebuild forecast_baseline_template.xlsx (v3.4) for Lele Aviation.
Carries forward everything from v3.3.2, plus:
  - Retirement age defaults updated to Jon's researched values (Widebody 20,
    Narrowbody 25, Regional 30, Turboprop 30) -- pushed to the DB via
    update_retirement_age_defaults.py; 08_Retirement_Defaults just reflects
    the DB as always.
  - IFE is now tracked PER CABIN CLASS instead of one whole-subfleet pair.
    AeroLOPA's source data only distinguishes IFE at the 4-way cabin-class
    grain (First / Business / Premium Economy / Economy), not the finer
    seat-hardware-variant grain already used for seat counts -- confirmed
    with Jon directly (2026-06-22). 01_Fleet_Baseline cols AK-AR replace the
    old single Has IFE?/IFE Type pair (AK/AL) with FOUR pairs, one per
    cabin class. Has Connectivity?/Connectivity Notes shift right from
    AM/AN to AS/AT (net +6 columns: +8 new IFE cols, -2 old IFE cols).
...
OUT_PATH = "/sessions/trusting-amazing-albattani/mnt/Forecast Files/forecast_baseline_template_v3.4.xlsx"
# Source for carry-forward fields with no DB backing (curated Include Forecast?
# flags, free-text Fleet Notes, free-text Cadence Notes) -- use Jon's latest
# uploaded file (his retirement-age edits, already pushed to the DB above, are
# not read from here; this file is used purely for the three sidecar fields).
SOURCE_PATH = "/sessions/trusting-amazing-albattani/mnt/uploads/forecast_baseline_template_v3.3-c9ebeae8.xlsx"
...
cur.execute("""
    select s.id, a.name, t.model, t.category, s.subfleet_name, s.deck, s.source,
           ...
           s.retirement_age_override,
           s.has_ife_first, s.ife_type_first,
           s.has_ife_business, s.ife_type_business,
           s.has_ife_premium_economy, s.ife_type_premium_economy,
           s.has_ife_economy, s.ife_type_economy,
           s.has_connectivity, s.connectivity_notes
    from subfleets s
...
cur.execute("select name, airline_category from airlines order by name")
_airline_rows = cur.fetchall()
all_airlines = [row[0] for row in _airline_rows]
airline_category_db = {row[0]: row[1] for row in _airline_rows}
assert len(all_airlines) == 987, f"expected 987 airlines, got {len(all_airlines)}"
_n_categorized = sum(1 for v in airline_category_db.values() if v)
assert _n_categorized == 987, (
    f"expected all 987 airlines categorized, got {_n_categorized} "
    f"(v3.3.2: full categorization pass should be complete)"
)

cur.close()
conn.close()

# Carry-forward sidecar fields with no DB backing -- re-derived from Jon's
# latest uploaded workbook (see SOURCE_PATH comment above).
...
for _r in range(3, _ws_ad.max_row + 1):
    ...
    # Category (col B) is sourced directly from airlines.airline_category (DB), not the sidecar.
    if _notes:
        airline_notes_filled[_airline] = _notes
...
print(f"Loaded: {len(subfleets)} subfleets, {len(rb)} retrofit_baseline, "
      ...
      f"{len(curated_airlines)} curated airlines, {_n_categorized} airlines categorized (DB-sourced), "
      f"{len(airline_notes_filled)} cadence-note carryovers, {len(fleet_notes)} fleet notes")
...
ws["B1"] = "LELE AVIATION INTELLIGENCE PLATFORM — Forecast Baseline Template v3.4"
...
sheet_purpose = [
    ...
    ("05_Airline_Defaults", "One row per airline (987). Category is pre-populated from the database (987/987, green = DB-sourced/editable) -- override only if you want a different value for a specific airline. Airline-specific retrofit overrides (Whole Aircraft / Premium / Economy) remain optional inputs here."),
    ...
    ("08_Retirement_Defaults", "Default retirement age by Aircraft Size Class (4 rows: Widebody/Narrowbody/Regional/Turboprop). Feeds 01_Fleet_Baseline's Effective Retirement Age cascade. Edit values freely."),
]
r = 5
...
r += 1
ws.cell(row=r, column=2, value="WHAT CHANGED IN v3.4")
...
changelog_v34 = [
    "Retirement age defaults updated to Jon's researched values: Widebody 25 -> 20, Regional 20 -> 30 "
    "(Narrowbody stays 25, Turboprop stays 30). Note: the Notes column on 08_Retirement_Defaults still says "
    "Regional jets are \"often retired/parted-out earlier than mainline narrowbody\" -- worth a look, since "
    "30 is now the highest of the four values, not the lowest. Flagging in case the note (not the number) "
    "needs updating, or the number needs a second pass.",
    "IFE is now tracked PER CABIN CLASS instead of one value for the whole subfleet. The old Has IFE?/IFE "
    "Type pair (cols AK/AL) is replaced by FOUR pairs -- one each for First, Business, Premium Economy, and "
    "Economy (cols AK-AR). This is the finest grain AeroLOPA's source data actually supports: it distinguishes "
    "IFE by cabin class, not by the seat-hardware sub-variant (e.g. it doesn't say whether F Lie-Flat and F "
    "Recliner seats have different IFE -- both draw from the same First-cabin value). Confirmed this design "
    "with Jon directly.",
    "Has Connectivity?/Connectivity Notes shift from cols AM/AN to AS/AT to make room (net +6 columns: 8 new "
    "IFE columns minus the 2 retired ones). No change to their behavior -- still 100% manual entry, no source "
    "data exists for connectivity anywhere yet.",
    "Each of the 4 new Has-IFE? columns uses NULL/blank for \"not covered by AeroLOPA or this subfleet has no "
    "seats of this class\" (yellow), distinct from FALSE/\"No-IFE\" which means \"AeroLOPA confirms this cabin "
    "has no IFE\" (green). This is a cleaner signal than the old single pair, which conflated those two cases.",
    "Auto-drafted from AeroLOPA's per-cabin IFE Description text for the 1,511 AeroLOPA-source subfleets, same "
    "classifier as before (AVOD-Seatback / Overhead / BYOD-Streaming / No-IFE). Coverage by class, across all "
    "2,753 active subfleets: First 78 covered (70 have IFE), Business 932 covered (607 have IFE), Premium "
    "Economy 264 covered (209 have IFE), Economy 1,477 covered (681 have IFE). Manual-source subfleets (1,242 "
    "of the 2,753 active rows) have no AeroLOPA coverage at all, so stay blank across all 4 classes, same as "
    "before.",
]
for i, txt in enumerate(changelog_v34, start=1):
...
changelog_v33 = [
    "Aircraft retirements are now modeled. 08_Retirement_Defaults sheet sets a default retirement age per "
    "Size Class (Widebody/Narrowbody/Regional/Turboprop). 01_Fleet_Baseline cols AH-AJ add a per-subfleet "
    "override cascade (subfleet override > Size Class default) plus a calculated Est. Retirement Start Year, "
    "the first forecast year a subfleet's average age is projected to cross its effective retirement age.",
    ...
    "netted against scheduled deliveries.",
    "v3.3.1: Turboprop subfleets (73 active) now have full pricing coverage (72 rows added to assumptions). "
    "03_Pricing_Reference now totals 486 rows.",
    "v3.3.2: All 987 airlines now have a Category assigned (was 23/987) -- Jon's full manual categorization "
    "pass. 05_Airline_Defaults col B is now sourced directly from airlines.airline_category in the DB.",
]
...
col_defs = [
    ...
    ("has_ife_<class>? / ife_type_<class> (x4: First/Business/Prem Eco/Economy)", "Drafted from AeroLOPA where covered (green); blank where not covered (yellow -- needs your input). Grain is cabin class, not seat-hardware variant -- AeroLOPA doesn't distinguish further. IFE Type values: AVOD-Seatback / Overhead / BYOD-Streaming / No-IFE."),
...
COLS = [
    ...
    "Has IFE? (First)", "IFE Type (First)",                                                     # AK-AL
    "Has IFE? (Business)", "IFE Type (Business)",                                               # AM-AN
    "Has IFE? (Prem Eco)", "IFE Type (Prem Eco)",                                                # AO-AP
    "Has IFE? (Economy)", "IFE Type (Economy)",                                                 # AQ-AR
    "Has Connectivity?", "Connectivity Notes",                                                   # AS-AT
]
assert len(COLS) == 46
...
group_headers = [
    ...
    ("AK1:AR1", "IFE BY CABIN CLASS (First / Business / Prem Eco / Economy)"),
    ("AS1:AT1", "CONNECTIVITY"),
]
...
widths = {"A":28,"B":16,"C":13,"D":20,"E":9,"F":8,"G":8,"H":8,"I":8,"J":8,"K":9,"L":8,"M":8,
          ...
          "AE":14,"AF":14,"AG":30,
          "AH":13,"AI":13,"AJ":14,
          "AK":12,"AL":15,"AM":12,"AN":15,"AO":12,"AP":15,"AQ":12,"AR":15,
          "AS":14,"AT":30}
...
dv_ife_type.add(f"AN3:AN{last_row}")
dv_yn.add(f"AO3:AO{last_row}")
dv_ife_type.add(f"AP3:AP{last_row}")
dv_yn.add(f"AQ3:AQ{last_row}")
dv_ife_type.add(f"AR3:AR{last_row}")
dv_yn.add(f"AS3:AS{last_row}")
...
for (sf_id, airline, model, size_class, subfleet_name, deck, source, f_lf, f_rc, f_dom, j_lf, j_rc, j_pd,
     pe, ep, eco, retirement_age_override,
     ...
     has_connectivity, connectivity_notes) in subfleets:
    ...
    # --- IFE by cabin class (AK-AR) ---
    # has_ife_<class>/ife_type_<class>: drafted from AeroLOPA per-cabin Description where it covers this
    # subfleet (green); left blank where uncovered (yellow -- flagged for manual review, never guessed).
    ife_cols = [
    ...
    col_i = 37  # AK
    for has_ife, ife_type in ife_cols:
        ife_yn = "YES" if has_ife is True else ("NO" if has_ife is False else None)
        c = ws.cell(row=row, column=col_i, value=ife_yn)
        style_cell(c, F_GREEN if has_ife is not None else F_YELLOW)
        c = ws.cell(row=row, column=col_i + 1, value=ife_type if ife_type else None)
    ...
    c = ws.cell(row=row, column=45, value=None)
    style_cell(c, F_YELLOW)
    c = ws.cell(row=row, column=46, value=None)
    style_cell(c, F_YELLOW, align="left", wrap=True)
...
plain(ws.cell(r,column=2), size=12, bold=True, color="FF1F3864")
r += 1
...
steps = [
    ...
    ("RESOLVED v3.3.2 — review the values", "05_Airline_Defaults col B (Category, FSC/GL/LCC/ULCC/RFSC/REG/"
     "LEI/OTH) is now pre-populated for all 987 airlines, sourced live from airlines.airline_category in the "
     "DB (green fill, editable) -- Jon's full categorization pass. Edit a cell to override; on the next "
     "rebuild your edit will be replaced by whatever's in the DB at that time, so push category corrections "
     "back to the DB (or flag to Claude) rather than relying on a workbook-only edit surviving a rebuild."),
    ...
    ("OPTIONAL — only for subfleets you know differ", "Retirement Age Override (col AH). Leave blank to use the "
     ...
     "don't edit directly."),
    ("RESOLVED v3.4 — review the values", "08_Retirement_Defaults' 4 retirement-age numbers (Widebody/"
     "Narrowbody/Regional/Turboprop) now reflect Jon's researched assumptions: 20/25/30/30 years. Note: the "
     "Notes column there still frames Regional as retiring earlier than Narrowbody, which the new numbers no "
     "longer reflect (30 > 25) — worth a look. This also drives the retirements / fleet_count_adjusted columns "
     "in the underlying forecast data (not shown directly in this template) — once a subfleet's average age "
     "crosses its effective retirement age, 20% of the fleet phases out per year against a 5-year retirement "
     "window."),
    ("REVIEW — auto-drafted, not guessed, NEW GRAIN IN v3.4", "Has IFE? / IFE Type are now FOUR pairs, one per "
     "cabin class (cols AK-AR: First, Business, Prem Eco, Economy) instead of one pair for the whole subfleet. "
     "Auto-drafted from AeroLOPA's per-cabin Description text wherever AeroLOPA covers that cabin (green fill); "
     "blank (yellow) where AeroLOPA doesn't cover that cabin class for this subfleet, including all "
     "Manual-source subfleets. Coverage: First 78/2,753, Business 932/2,753, Prem Eco 264/2,753, Economy "
     "1,477/2,753 — fill in the rest only if you know the actual IFE system; otherwise leave for a future data "
     "refresh."),
    ("MANDATORY — no source data exists", "Has Connectivity? / Connectivity Notes (cols AS/AT, moved from "
     "AM/AN in v3.4 to make room for the IFE-by-cabin-class columns) are blank for every row — there is no "
     "WiFi/connectivity data source anywhere in the pipeline yet. 100% manual entry if you want this tracked; "
...
ws["A1"] = f"AIRLINE DEFAULTS  —  All {len(all_airlines)} Airlines  |  Category (DB-sourced) + Per-Track Retrofit-Year Overrides"
style_title(ws["A1"], size=11)
for col in "BCDEF":
    style_title(ws[f"{col}1"])
...
for airline in all_airlines:
    category = airline_category_db.get(airline)
    notes = airline_notes_filled.get(airline)
